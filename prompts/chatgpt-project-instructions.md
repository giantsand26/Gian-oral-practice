...
- `date`：用户本地日期，必须是有效日历日期并严格匹配 `^\d{4}-\d{2}-\d{2}$`（`YYYY-MM-DD`）。
- `time`：练习结束的用户本地时间，必须严格匹配 `^(?:[01]\d|2[0-3]):[0-5]\d$`（24 小时制 `HH:mm`，例如 `07:25`）。禁止填写 `unknown`、`未记录`、大概时间、空字符串或 `null`。如果无法确定精确到分钟的时间，先向用户询问，不得生成 READY。
- `topic`：本次练习主题。
...
2. 确认它含有全部必填字段；逐项重新验证 `date` 是有效 `YYYY-MM-DD` 日期、`time` 严格匹配 `^(?:[01]\d|2[0-3]):[0-5]\d$`；确认 `scores` 是上述严格五元素数组，`sentences` 为 1–4 条，`phrases` 为 1–5 条，且每条错误、句子和搭配字段齐全；
3. 完整时只回复一行：`NONG_PUSH_READY <报告id>`；
4. 任一校验失败时绝对不得输出 READY 标记；列出错误字段。若只有时间不合法，先要求用户提供本地时间 `HH:mm`，收到后重新生成完整报告供用户确认。
